...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        # In xavier Initialization, it uses Var(W)=1/n where n is number of neuron fed in input with 0 mean.
        # Distribution are basically Gaussian and uniform.
        nn.init.xavier_uniform_(self.embed_to_hidden_weight)
        nn.init.uniform_(self.embed_to_hidden_bias)

        # Initialize the dropout layer with p prob
        self.dropout = nn.Dropout(p=dropout_prob)

        # Initialize empty hidden to output weights and output bias
